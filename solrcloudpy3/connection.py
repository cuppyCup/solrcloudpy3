...
import solrcloudpy3.collection as collection
from solrcloudpy3.utils import _Request
    ...
    def cluster_health(self):
        ...
        if semver.match(self.version, '<5.4.0'):
            ...
            for coll in collections:
                ...
                for shard, shard_info in iter(shards.items()):
                    replicas = shard_info['replicas']
                    for replica, info in iter(replicas.items()):
                        state = info['state']
                        if state != 'active':
